...
def smarty_run(qb_id, qb_vendor, qb_company, qb_street_address, qb_city, qb_state, qb_zip):
    ...
    input_id = str(hash(qb_vendor))

    lookup = StreetLookup()
    lookup.input_id = input_id
    lookup.addressee = qb_company
    lookup.street = qb_street_address
    lookup.street2 = "" # tbd if needed
    lookup.secondary = ""
    lookup.urbanization = ""  # Only applies to Puerto Rico addresses
    lookup.city = qb_city
    lookup.state = qb_state
    lookup.zipcode = qb_zip
    lookup.candidates = 3
